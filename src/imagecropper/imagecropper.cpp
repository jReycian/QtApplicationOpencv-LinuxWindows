...
{
    // calculate moving area of rectangle
...
{
    // start crop image base on rectangle area
...
{
    QPointF mousePos = _event->pos(); // relative to itself (widget)
	//
	if (!pimpl->isMousePressed) {
        // Normal state processing, i.e. size does not change
        // cropping area and it doesn't move around the widget
        ...
        // Handling actions on the crop area
        // ... determine the offset of the mouse cursor
		QPointF mouseDelta;
		...
		if (pimpl->cursorPosition != CursorPositionMiddle) {
			...
			QRectF newGeometry =
					calculateGeometry(
						...
						mouseDelta);
            // ... the user tries to turn the cropped area inside out
...
namespace {
    // Is the point near the side coordinate
		...
		)
{
    ...
    // If the user is trying to turn the cropped area inside out,
